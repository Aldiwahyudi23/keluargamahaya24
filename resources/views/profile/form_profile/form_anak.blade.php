...
$cek_user = User::where('data_warga_id', $anak->id);
?>
<div class="row">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="text-center">
                        <a href="{{ asset( $foto_user_anak->foto) }}" data-toggle="lightbox" data-title="Foto Profile {{ Auth::user()->name }}" data-gallery="gallery" data-footer=' <form action="{{ Route('data-warga.update', Crypt::encrypt($anak->id)) }}" method="post" enctype="multipart/form-data">
                            ...
                            <img src="{{ asset( $foto_user_anak->foto) }}" width="130px" class="profile-user-img img-fluid img-circle" alt="User profile picture">
                        </a>
                    </div>
                    ...
                    <form action="{{Route('data-warga.store_user')}}" method="POST" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="email" id="email" value="{{$anak->email}}">
                        <input type="hidden" name="no_hp" id="no_hp" value="{{$anak->no_hp}}">
                        <input type="hidden" name="name" id="name" value="{{$anak->nama}}">
                        <input type="hidden" name="name" id="name" value="{{$anak->nama}}">
                        <input type="hidden" name="data_warga_id" id="data_warga_id" value="{{$anak->id}}">
                    ...
                    <ul class="list-group list-group-unbordered mb-3">
                        <li class="list-group-item">
                            ...
                            @else
                            <a class="float-right">{{ $anak->email }}</a>
                            @endif
                        </li>
                        <li class="list-group-item">
                            <b>No HP</b> <a class="float-right">{{ $anak->no_hp }}</a>
                        </li>
                    </ul>
...
<form action="{{ route('data-warga.update',Crypt::encrypt($anak->id)) }}" method="post" enctype="multipart/form-data">
    @method('PATCH')
    {{csrf_field()}}
    <div class="row">
        <div class="col-12 col-sm-4">
            <div class="card card-primary card-outline card-outline-tabs">
                <div class="card-body">
                    <center>
                        <h5 class="text-bold card-header bg-light p-0"> EDIT DATA anak</h5>
                    </center>
                    <hr>
                    <div class="form-group row">
                        <label for="nama">Nama Lengkap</label>
                        <input type="text" id="nama" name="nama" value="{{ old('nama',$anak->nama) }}" placeholder="Nama Lengkap" class="form-control col-12 @error('nama') is-invalid @enderror">
                        @error('nama')
                    ...
                    <div class="form-group">
                        ...
                        <select id="jenis_kelamin" name="jenis_kelamin" class="select2bs4 form-control col-12 @error('jenis_kelamin') is-invalid @enderror">
                            @if(old('jenis_kelamin',$anak->jenis_kelamin) == true)
                            <option value="{{old('jenis_kelamin',$anak->jenis_kelamin)}}">{{old('jenis_kelamin',$anak->jenis_kelamin)}}</option>
                            @endif
                    ...
                    <div class="form-group row">
                        ...
                        <input type="text" id="tempat_lahir" name="tempat_lahir" value="{{ old('tempat_lahir',$anak->tempat_lahir) }}" placeholder="Masukan Tempat Anda Lahir" class="form-control col-12 @error('tempat_lahir') is-invalid @enderror">
                        @error('tempat_lahir')
                        <div class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="{{ old('tanggal_lahir',$anak->tanggal_lahir) }}" placeholder="tanggal_lahir Sub Menu" class="form-control col-12 @error('tanggal_lahir') is-invalid @enderror">
                        @error('tanggal_lahir')
                        <div class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </div>
            ...
            <div class="card card-primary card-outline card-outline-tabs">
                <div class="card-body">
                    ...
                    <div class="form-group row">
                        ...
                        <input type="number" id="no_hp" name="no_hp" value="{{ old('no_hp',$anak->no_hp) }}" placeholder="081xx xxxx xxxx" class="form-control col-12 @error('no_hp') is-invalid @enderror">
                        @error('no_hp')
                        <div class="invalid-feedback">
                            <strong>{{ $message }}</strong>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="agama" name="agama" class="select2bs4 form-control @error('agama') is-invalid @enderror">
                            @if(old('agama',$anak->agama) == true)
                            <option value="{{old('agama',$anak->agama)}}">{{old('agama',$anak->agama)}}</option>
                            @endif
                    ...
                    <div class="form-group">
                        ...
                        <select id="status_pernikahan" name="status_pernikahan" class="select2bs4 form-control @error('status_pernikahan') is-invalid @enderror">
                            @if(old('status_pernikahan',$anak->status_pernikahan) == true)
                            <option value="{{old('status_pernikahan',$anak->status_pernikahan)}}">{{old('status_pernikahan',$anak->status_pernikahan)}}</option>
                            @endif
                    ...
                    <div class="form-group">
                        ...
                        <select id="status" name="status" class="select2bs4 form-control @error('status') is-invalid @enderror">
                            @if(old('status',$anak->status) == true)
                            <option value="">{{old('status',$anak->status)}}</option>
                            @endif
                            <option value="">-- Pilih Status Pernikahan --</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" id="email" value="{{$anak->email}}" class="form-control col-12" placeholder="Jika Ingin Mengaktifkan Akun/ bisa Login ke aplikasi , Harap Masukan Email">
                    </div>
                    <hr>
